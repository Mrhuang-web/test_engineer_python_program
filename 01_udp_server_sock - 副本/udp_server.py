...
def initialize_heartbeat_sender(heartbeat_config):
    clients = set()
    heartbeat_sender = HeartbeatSender(clients, heartbeat_config.get('interval', 60), heartbeat_config.get('hex'))
...
async def main():
    ...
    # 初始化心跳包发送器
    heartbeat_sender = initialize_heartbeat_sender(heartbeat_config)
