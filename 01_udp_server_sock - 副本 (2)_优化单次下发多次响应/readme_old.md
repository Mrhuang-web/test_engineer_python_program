...
通信流程：
    客户端启动并向服务器发送初始数据包，服务器接收到数据包后，将客户端地址添加到 clients 集合中
    服务器定期遍历 clients 集合，使用 sock 向每个客户端地址发送心跳包
    客户端接收到心跳包后，可以选择回复一个响应包，服务器根据响应包判断客户端是否在线
    如果某个客户端在一定时间内没有响应心跳包，服务器可以将其从 clients 集合中移除，认为该客户端已离线
    
    服务器监听自己的接口（如 0.0.0.0:10020）是监听来自任何客户端的请求
    0.0.0.0 是一个特殊的 IP 地址，表示服务器会监听所有可用的网络接口上的请求
        比如：服务器监听 0.0.0.0:10020，
            这意味着它会接收来自任何网络接口（如以太网、Wi-Fi 等）的请求
            包括来自本地机器的请求（通过 127.0.0.1）和来自外部网络的请求（通过服务器的实际 IP 地址）
    客户端如何与服务器通信
        客户端需要知道服务器的 IP 地址和端口号，以便向服务器发送请求（即sock端口 - 也就是fsu模拟器端口和ip）
        客户端的代码通常会指定服务器的地址和端口
    服务器如何知道客户端的地址
        当客户端向服务器发送请求时，服务器会从接收到的数据包中提取客户端的 IP 地址和端口号
        服务器可以使用这些信息来回复客户端，或者将其存储在 clients 集合中，以便后续发送心跳包
    通信流程：
        客户端向服务器的地址和端口发送请求。
        服务器接收到请求，获取客户端的地址。
        服务器根据客户端的地址发送响
